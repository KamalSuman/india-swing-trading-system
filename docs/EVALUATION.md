...
before any strategy, Kronos model, TradingAgents component, or benchmark can be
evaluated. It does not yet calculate returns or claim strategy performance.
...
A valid split plan alone is not a reportable backtest. Evaluation must still be
blocked until the following are implemented and supplied:

- a create-once trial registration made before holdout access;
